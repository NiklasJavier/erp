...
```
docker run -p 8080:8080 erp:latest
```
...
If the pods say `Container image "erp:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
